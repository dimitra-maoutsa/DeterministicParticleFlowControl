...
- `f_true`   : function handler of system drift function when  the reweighted Brownian bridge functionality for forward sampling is used. In that case, `f` should be supplied with the drift function of the brownian bridge, `U` should be set to the necessary path constraint (see [paper](https://journals.aps.org/prresearch/abstract/10.1103/PhysRevResearch.4.043035?ft=1)), and `reweight` and `brown_bridge` should be turned to True,
...
[paper](https://journals.aps.org/prresearch/abstract/10.1103/PhysRevResearch.4.043035?ft=1). To cite it:
