...
def torched_score_function_multid_seperate_all_dims(X: np.ndarray, Z: np.ndarray,
    ...
    if func_out==False: #if output wanted is evaluation at data points
        ...
        res1 = -K_xz @ torch.linalg.inv( C*torch.eye(M, M, device=device) + Ksinv @ A + 1e-3 * torch.eye(M, device=device))@ Ksinv@sumgradx_K
        ...
        res1 = lambda x: K_sz(x) @ (-torch.linalg.inv(C*torch.eye(M, M, device=device) + Ksinv @ A + 1e-3 * torch.eye(M, device=device)) ) @ Ksinv@sumgradx_K
...
ln1 = score_function_multid_seperate_all_dims(X2, Z.T, func_out=False, C=0.001,
                                              kern='RBF', l=lengthsc)
ln_torch = torched_score_function_multid_seperate_all_dims(torch.tensor(X2, device=DEVICE),
                                                           torch.tensor(Z.T, device=DEVICE),
                                                           func_out=False,
                                                           C=0.001, kern='RBF',
                                                           l=lengthsc,
                                                           device=DEVICE)
