...
for one in range(1,2):
    ...
    for i in range(6):
        ...
        if i<3:
            ...
            ax.set_xlabel(r'model',fontsize=26)
        if i in [1,2,4,5]:
            ax.set_ylabel('')
        else:
            if one==0:
                
                ax.set_ylabel(r'control'+'\n'+r'\fontsize{16pt}{3em}\selectfont{}{$ \| u(x,t) \|_2^2$}', 
                              multialignment='center',fontsize=26,
                             linespacing=0.75)
            elif one==1:
                
                ax.set_ylabel(r'terminal'+'\n'+r'error'+'\n'+r'\fontsize{16pt}{3em}\selectfont{}{$ (x^*- X_T)^2$}', 
                              multialignment='center',fontsize=26,
                             linespacing=0.75)
            elif one==2:
               
               ax.set_ylabel(r'path'+'\n'+r'error'+'\n'+r'\fontsize{16pt}{3em}\selectfont{}{$\int( X_t - y^*)^2\mathrm{d}t$}', 
                             multialignment='center',fontsize=26,
                             linespacing=0.75)
        if i<3:
            if one == 0:
        ...
        if i != 3:
            ...
            leg3 = ax.legend(handles, labels, title='constraints',
                             ...
                             loc=2, ncol=1, frameon=True,fontsize = 'large',
                             shadow=None,framealpha =0,edgecolor ='#0a0a0a')
            for text in leg3.get_texts():
                text.set_color('#4f4949')
            plt.setp(leg3.get_title(),fontsize='large', color='#4f4949')
    ...
    plt.savefig('Gaussian_evolitionary'+which[one]+'.png', bbox_inches='tight',
                dpi=300 , pad_inches = 0.1, transparent='False',  
                facecolor='white')
    plt.savefig('Gaussian_evolitionary'+which[one]+'.pdf', bbox_inches='tight',
                dpi=300,  pad_inches = 0.1, transparent='False',  
                facecolor='white')
