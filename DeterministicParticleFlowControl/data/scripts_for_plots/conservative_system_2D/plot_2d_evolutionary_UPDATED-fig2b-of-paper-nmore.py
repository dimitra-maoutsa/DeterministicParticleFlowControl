...
leg1 = ax0.legend(handles2b, labels2b, title=None,
          ...
          bbox_to_anchor=[-0.55, 0.30955],labelspacing = 0.3,
          loc=2, ncol=1, frameon=True,fontsize = 'small',shadow=None,
...
leg2 = ax0.legend(handles2, labels2, title=None,
          handletextpad=0.5, columnspacing=3.2,handlelength=0.8,
          bbox_to_anchor=[-0.55, 1.0],labelspacing = 0.3,
          loc=2, ncol=1, frameon=True,fontsize = 'small',shadow=None,
          framealpha =0,edgecolor ='#0a0a0a')
...
leg4 = ax2.legend(handles, labels, title=None,
          ...
          loc=2, ncol=2, frameon=True,fontsize = 'small',shadow=None,
          framealpha =0,edgecolor ='#0a0a0a')#,bbox_transform=fig.transFigure)
...
#plt.tight_layout()
plt.show()
plt.savefig(savefolder+'path_2d_pheno.png', bbox_inches='tight',dpi=300 ,
            pad_inches = -0, transparent='False',  facecolor='white')
plt.savefig(savefolder+'path_2d_pheno.pdf', bbox_inches='tight',dpi=300,
            pad_inches = -0, transparent='False',  facecolor='white')
...
plt.savefig(savefolder+'App.with_path_2d_pheno_forward_particles_backward.png',
            bbox_inches='tight',dpi=300 , pad_inches = 1, transparent='False',
            facecolor='white')
plt.savefig(savefolder+'App.with_path_2d_pheno_forward_particles_backward.pdf',
            bbox_inches='tight',dpi=300,  pad_inches = 1, transparent='False',
            facecolor='white')
...
#  Varying density along a streamline
ax0 = fig.add_subplot(gs[0, 0])
plt.contourf(X, Y, speed.reshape(X.shape)  , cmap=plt.cm.BuPu_r,
             levels=150,alpha=0.65)
...
#ax0.plot( (bridg2d.Z[0]), (bridg2d.Z[1]),alpha=0.31, c='grey',lw=1) '#C70039'
ax0.plot( np.mean(bridg2dB[0],axis=0), np.mean(bridg2dB[1],axis=0),'.',
         alpha=0.85, c='#900C3F',lw=2.5,label= r'$\mu_{q_{t}(x)}$') #cm.viridis(0.97)
ax0.plot( np.mean(bridg2dB[0],axis=0)+ np.std(bridg2dB[0],axis=0),
         np.mean(bridg2dB[1],axis=0) + np.std(bridg2dB[1],axis=0),'--',
         alpha=0.85,c='#900C3F',lw=2.5,label= r'$\sigma_{q_{t}(x)}$')
ax0.plot( np.mean(bridg2dB[0],axis=0)- np.std(bridg2dB[0],axis=0),
...
#ax0.plot( (bridg2d.Z[0]), (bridg2d.Z[1]),alpha=0.31, c='grey',lw=1)
ax2.plot( np.mean(Fcont[0],axis=0), np.mean(Fcont[1],axis=0),'-',alpha=0.85,
         c=cm.viridis(0.97),lw=2.5,label= r' $\mu_{\hat{q}^{DPF}_{t}}$')
ax2.plot( np.mean(Fcont[0],axis=0)+ np.std(Fcont[0],axis=0),
         np.mean(Fcont[1],axis=0) + np.std(Fcont[1],axis=0),
         linestyle=(0, (1.5, 3)),alpha=0.85,c=cm.viridis(0.97),
         lw=2.5,label= r'$\sigma_{\hat{q}^{DPF}_{t}}$')
ax2.plot( np.mean(Fcont[0],axis=0)- np.std(Fcont[0],axis=0),
         np.mean(Fcont[1],axis=0) - np.std(Fcont[1],axis=0),
...
dpf_df = pd.DataFrame( columns = ['control', 'M','N','rep_u',
...
for ni,N in enumerate(Ns):
    for rep in range(1):
        for mi,M in enumerate(Ms):
            ...
            df1 =  pd.DataFrame(data, columns = ['control', 'M','N','rep_u',
                                                 'path_error', 'end_error'])
...
for ti in range(0,timegrid.size):
    if ti==timegrid.size-200:
        ...
        ax1.plot(Fnon[0,0,ti:ti+2],Fnon[1,0,ti:ti+2],
                 c=cm.copper(0.2+ti/(0.2+timegrid.size)), alpha=0.85,
                 lw=1.8, label=r'$X^{\mathrm{unc}}_t$')
...
plt.plot(timegrid, np.mean(Fcont[0],axis=0), linestyle=(0, (3, 3)),
         c=my_mag, lw=4, zorder=2,label=r'$\mu_{t}^{DPF}$')
plt.plot(timegrid[::1], np.mean(Fcont_pice[ni_pice][0, :,::1],axis=0),'-',
         c='grey',lw=4.,zorder=0,label=r'$\mu_{t}^{\mathrm{pice}}$')
#plt.plot(timegrid,np.mean(Fcont_pice[ni_pice][0],axis=0),linestyle=(0, (1, 4)), c='#4f4949',lw=4,dash_capstyle='butt')
plt.plot(timegrid, np.mean(Fnon[0], axis=0), '-', c=orag,lw=2, zorder=0,
         alpha=0.95,label=r'$\mu_{t}^{\mathrm{unc}}$')
plt.plot(timegrid[::20],
         np.mean(Fcont[0,:,::20],axis=0) - np.std(Fcont[0,:,::20],axis=0),'.',
         c=my_mag,lw=4 ,label=r'$\sigma_{t}^{\mathrm{DPF}}$')
plt.plot(timegrid[::20],
         np.mean(Fcont[0,:,::20],axis=0) + np.std(Fcont[0,:,::20],axis=0), '.',
         c=my_mag,lw=4)
plt.plot(timegrid,
         np.mean(Fcont_pice[ni_pice][0],axis=0)-np.std(Fcont_pice[ni_pice][0],
                                                       axis=0),
         linestyle=(0, (6, 5)),c='grey', lw=4,dash_capstyle='butt',zorder=0,
         label=r'$\sigma_{t}^{\mathrm{pice}}$')
plt.plot(timegrid, np.mean(Fcont_pice[ni_pice][0],
                          axis=0)+np.std(Fcont_pice[ni_pice][0],axis=0),
         linestyle=(0, (6, 5)), c='grey', lw=4, dash_capstyle='butt', zorder=0)
...
plt.plot(timegrid,np.mean(Fcont[1],axis=0),linestyle=(0, (3, 3)), c=my_mag,
         lw=4, zorder=2)
...
plt.plot(timegrid[::20],np.mean(Fcont[1,:,::20],axis=0)-np.std(Fcont[1,:,::20],
                                                               axis=0),
         '.', c=my_mag,lw=4,label=r'$\sigma_{t}^{\mathrm{DPF}}$')
plt.plot(timegrid[::20],np.mean(Fcont[1,:,::20],axis=0)+np.std(Fcont[1,:,::20],
                                                               axis=0),
...
"""
##########################################################################
gs01 = gridspec.GridSpecFromSubplotSpec(2, 1, subplot_spec=gs[0:2, 2],
                                        wspace=0.5)
...
controls = np.log(np.array([np.power(np.sum(np.sqrt(np.nansum(np.power(used_u[:,:,:-1],2),
                                                              axis=0)),
                                            axis=-1),2) /(T/dt),
                            np.power(np.sum(np.sqrt(np.nansum(np.power(used_upice[ni_pice][:,:,:-1],2), axis=0)),
                                            axis=-1),2) /(T/dt)]) )
import pandas as pd
...
ax10.set_ylabel(r'control$ $'+'\n'+r' \fontsize{14pt}{3em}\selectfont{}{$\log \, \| u(x,t) \|_2^2$}',
                multialignment='center',fontsize=24, linespacing=0.75)
ax10.set_xlabel(r'')
ax10.tick_params(axis="both",direction="in", bottom=True, left=True,top=True,
...
ax11 = fig.add_subplot(gs01[1], sharex=ax10)
#fig.add_subplot(gs[1, 3], sharex=ax10)
...
ax11.tick_params(axis="both",direction="in", top=True, left=True,bottom=True,
                 right=True)
...
ax11.tick_params(axis="both", which='minor',direction="in", bottom=True,
                 left=True,top=True, right=True, color='#4f4949')
plt.ylim(0.02,0.12)
...
plt.show()
plt.savefig(savefolder+'Evolutionary_with_path_2d_pheno_finalb.png',
            bbox_inches='tight',dpi=300 , pad_inches = 0., transparent='False',
            facecolor='white')
plt.savefig(savefolder+'Evolutionary_with_path_2d_pheno_finalb.pdf',
            bbox_inches='tight',dpi=300,  pad_inches = 0., transparent='False',
            facecolor='white')
